    ...
    def api_guardar_registros(
        ...
        **kwargs,
          ...
          "filas": [
            {
              ...
              "total_folios":      120,
              "total_escaneos":         130,
              "tipo_escaner_id":     [1, 2],   ← lista de IDs (Many2many)
        ...
        for idx, fila in enumerate(filas, start=1):
            ...
            tipo_escaner_id_raw = fila.get("tipo_escaner_id", [])
            # Compatibilidad: el JS envía tipo_escaner_id (singular, escalar)
            if not tipo_escaner_id_raw:
                single_id = fila.get("tipo_escaner_id")
                if single_id:
                    tipo_escaner_id_raw = [single_id]
            if not isinstance(tipo_escaner_id_raw, list):
                tipo_escaner_id_raw = (
                    [tipo_escaner_id_raw] if tipo_escaner_id_raw else []
                )
            # Many2many: [(6, 0, [id1, id2, ...])]
            tipo_escaner_m2m = [(6, 0, [int(x) for x in tipo_escaner_id_raw if x])]
            ...
            vals = {
                ...
                "tipo_escaner_id": tipo_escaner_m2m,
            }
